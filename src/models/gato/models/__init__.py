...
from .embedding import PatchPositionEncoding, ResidualEmbedding, LocalPositionEncoding, DiscreteEmbedding
...
class Gato(nn.Module):
    ...
    def __init__(self, config: Union[GatoConfig, Dict[str, Any]], **kwargs):
        ...
        self.img_conv = nn.Conv2d(3, 3, (1,1))

        self.text_models = [T5Adapter(None)]

        self.row_pos = [
        torch.tensor([[0/16, 1/16, 2/16, 3/16, 4/16, 5/16, 6/16, 7/16, 8/16, 9/16, 10/16, 11/16, 12/16, 13/16, 14/16, 15/16]*16], requires_grad=False),  # pos_from
        torch.tensor([[1/16, 2/16, 3/16, 4/16, 5/16, 6/16, 7/16, 8/16, 9/16, 10/16, 11/16, 12/16, 13/16, 14/16, 15/16, 1.00]*16], requires_grad=False)   # pos_to
        ]
        self.col_pos = [
        torch.tensor([[0/16, 1/16, 2/16, 3/16, 4/16, 5/16, 6/16, 7/16, 8/16, 9/16, 10/16, 11/16, 12/16, 13/16, 14/16, 15/16]*16], requires_grad=False),  # pos_from
        torch.tensor([[1/16, 2/16, 3/16, 4/16, 5/16, 6/16, 7/16, 8/16, 9/16, 10/16, 11/16, 12/16, 13/16, 14/16, 15/16, 1.00]*16], requires_grad=False)   # pos_to
        ...
        self.obs_pos = torch.linspace(0,256,steps=256+1,requires_grad=False).int().unsqueeze(0)
        self.obs_mask = torch.ones(256+1, requires_grad=False).int().unsqueeze(1)
    ...
    def forward(self, images, texts):
        ...
        # print(images.shape)
        for text_model in self.text_models:
            text_model.to(images.device)
        self.row_pos[0]=self.row_pos[0].to(images.device)
        self.row_pos[1]=self.row_pos[1].to(images.device)
        self.col_pos[0]=self.col_pos[0].to(images.device)
        self.col_pos[1]=self.col_pos[1].to(images.device)
        self.obs_pos=self.obs_pos.to(images.device)
        self.obs_mask=self.obs_mask.to(images.device)
