...
class RT1(nn.Module):
    def __init__(
        ...
        *,
        ...
        if use_mask:
            self.groundedSAM=GroundedSAM()
            self.conv_embed= nn.Sequential(
                nn.Conv2d(1, 8, kernel_size=4, stride=4),
                LayerNorm2d(8),
                nn.GELU(),
                nn.Conv2d(8, 32, kernel_size=4, stride=4),
                LayerNorm2d(32),
                nn.GELU(),
                nn.Conv2d(32, 128, kernel_size=2, stride=2),
                LayerNorm2d(128),
                nn.GELU(),
                nn.Conv2d(128, vit.embed_dim, kernel_size=1),
            )
    ...
    def forward(
        ...
        # cond_fns = (lambda x: x,)*len(cond_fns)
        ...
        if self.use_mask:
            masks=[]
            for image,instruction in zip(images,texts):
                mask=self.groundedSAM.predict(image,instruction).unsqueeze(0)
                masks.append(mask)
            masks=torch.cat(masks,dim=0)
            masks=masks.unsqueeze(1)
            # print(masks.shape)
            masks_embed=self.conv_embed(masks)
            tokens=tokens+masks_embed
...
if __name__=='__main__':
    ...
    video = torch.randn(2, 3, 1, 224, 224)
